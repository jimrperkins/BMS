...
## Parte 0: Preparar entorno de trabajo

En primer lugar, abriremos la máquina virtual desde el campus virtual. Seleccionaremos la máquina virtual con el *software* genérico. 

Una vez hecho esto, abriremos RStudio dentro de la máquina virtual (donde trabajaremos para que los resultados sean reproducibles) y, en la terminal ejecutaremos las siguientes dos líneas.
...
Una vez ejecutadas estas dos línas, pasamos a abrir un documento de tipo *R Markdown*. Para ello, simplemente hay que ir a *File* $>$ *New File* $>$ *R Markdown*. **Importante, nos pedirá descargar algunos paquetes, tenéis que darle el "ok".** Una vez descargados los paquetes que solicitaba el tipo de archivo, seleccionamos la configuración por defecto y obtendremos una plantilla de ejemplo. Como veremos, la comodidad de este tipo de archivos *R Markdown* reside en la capacidad de combinar texto libre con bloques de código que podemos ejecutar en cualquier momento. Para iniciar un bloque de código, puede hacerse con los comandos "Ctrl + Alt + i".
...
2. Caminos y Conectividad (Paths and Connectivity):  
  La existencia de caminos cortos y la conectividad entre nodos son cruciales para el clustering.  
  Los clusters suelen tener caminos más cortos entre sus nodos que con nodos externos, lo que refleja su cohesión.
  El largo de un camino se mide como el número de aristas (o la suma de pesos si es ponderado).  
  La longitud mínima entre dos nodos \\(i\\) y \\(j\\) es la distancia:

  $$
  d(i,j) = \min_{\pi \in P(i,j)} \, |\pi|,
  $$
  donde \\(P(i,j)\\) es el conjunto de todos los caminos entre \\(i\\) y \\(j\\), y $|\pi|$ es la longitud del camino $\pi$.

  La red se dice conexa si $d(i,j) < \infty$ para todo par de nodos \\(i,j\\).
  En grafos no conexos, el conjunto de nodos se divide en componentes conexas.

3. Centralidad (Centrality): La centralidad mide la importancia de un nodo dentro de la red. Hay varias formas de calcular la centralidad:

- Centralidad de Grado (Degree Centrality):
  Número de conexiones que tiene un nodo. Un alto grado puede indicar un nodo clave en la red.  
  Para un nodo $v$ en un grafo simple no dirigido:

  $$
  k(v) = \sum_{u \in V} a_{vu},
  $$

  donde $a_{vu}$ es la entrada de la matriz de adyacencia ($a_{vu} = 1$ si hay arista $v\text{--}u$, y 0 si no).

---

- Centralidad de Intermediación (Betweenness Centrality):
  Indica cuántas veces un nodo actúa como un puente a lo largo de los caminos más cortos entre otros nodos.  

  Sea $\sigma_{st}$ el número de caminos más cortos entre $s$ y $t$,  
  y $\sigma_{st}(v)$ cuántos pasan por $v$:
...
- Centralidad de Cercanía (Closeness Centrality): 
  Mide qué tan cerca está un nodo de todos los demás nodos en la red, lo que puede reflejar su capacidad para acceder rápidamente a información.  

  Con $d(v,t)$ la distancia más corta entre $v$ y $t$:

  $$
  C_C(v) = \frac{n-1}{\sum_{t \neq v} d(v,t)}.
  $$

  En grafos desconectados se recomienda la centralidad armónica:

  $$
  C_H(v) = \sum_{t \neq v} \frac{1}{d(v,t)},
  $$

  con la convención $1/\infty = 0$.

4.  Densidad (Network Density): Se refiere a la proporción de conexiones existentes en la red en comparación con el número máximo posible de conexiones. Una red densa puede indicar una alta interconexión entre nodos, lo cual es fundamental para la identificación de clusters.

5. Modularidad (Modularity):  La modularidad es una medida de la estructura de la red que indica la calidad de la división en módulos o clusters.  
  Un clustering con alta modularidad tiene muchas conexiones dentro de los clusters y pocas entre ellos.  
  La optimización de la modularidad es un objetivo común en muchos algoritmos de clustering.
...
```{r, eval=FALSE}
plot(g, vertex.size=degree(g)/2, edge.arrow.size=0.7, 
     vertex.label=NA, main="Degree")

plot(g, vertex.size=closeness(g)*2*10^3, edge.arrow.size=0.7, 
     vertex.label=NA, main="Closeness")

plot(g, vertex.size=betweenness(g)/30, edge.arrow.size=0.7, 
     vertex.label=NA, main="Betweenness")
```

```{r, eval=FALSE}
hist(degree(g), main="Degree", xlab="Grado", col="lightblue", border="white")
hist(closeness(g), main="Closeness", xlab="Cercanía", col="lightgreen", border="white")
hist(betweenness(g), main="Betweenness", xlab="Intermediación", col="salmon", border="white")
```
...
#### Generación de una Red tipo Erdős-Rényi
...
```{r, eval=FALSE}
hist(degree(g_ba), main="Degree", xlab="Grado", col="lightblue", border="white")
hist(closeness(g_ba), main="Closeness", xlab="Cercanía", col="lightgreen", border="white")
hist(betweenness(g_ba), main="Betweenness", xlab="Intermediación", col="salmon", border="white")
```
...
#### Aplicación de distintos algoritmos
...
#### Selección en base a métrica de calidad

En primer lugar, comprobamos qué proporción de los nodos de la red quedan efectivamente asignados a comunidades con cada algoritmo. Vemos el porcentaje de nodos cubiertos:

```{r, eval=FALSE}
total_nodes <- c(
Louvain = sum(sizes(cluster_louvain)),
GirvanNewman = sum(sizes(cluster_girvan_newman)),
Infomap = sum(sizes(cluster_infomap))
)
total_nodes <- 100*(total_nodes/gorder(g))
total_nodes
```

En todos los casos el valor debería rondar el 100 %, lo que indica que la partición cubre la totalidad de la red.

A continuación, calculamos la modularidad de cada partición, que mide la calidad de la división en comunidades (valores más altos indican que los nodos están más densamente conectados dentro de sus comunidades y menos conectados entre comunidades):

```{r, eval=FALSE}
mod_vals <- c(
Louvain = modularity(g, membership(cluster_louvain)),
GirvanNewman = modularity(g, membership(cluster_girvan_newman)),
Infomap = modularity(g, membership(cluster_infomap))
)
mod_vals
```
Con estos resultados podemos comparar cuál de los tres algoritmos ofrece la partición con mayor modularidad. Ese algoritmo sería, en principio, el más adecuado para describir la estructura de comunidades en nuestra red.

#### Filtrado

Es importante mencionar que normalmente se suele incorporar un paso de filtrado de clusters como etapa intermedia. En esta etapa seleccionamos aquellas comunidades encontradas con propiedades interesantes. Bien
a nivel de tamaño de su comunidad, alta conexión interna u otro tipo de métrica de calidad. 

Por fines didácticos esta es una etapa que se muestra en la práctica pero que no tendremos en cuenta para los estudios de enriquecimiento. Sin embargo, es bueno que los estudiantes lo tengan en cuenta si deseasen hacer estudios más finos en algún futuro.

```{r, eval=FALSE}
hist(sizes(cluster_louvain),
     breaks = seq(0, max(sizes(cluster_louvain))+2, by=2),
     col = "lightblue", border = "white",
     main = "Distribución de tamaños - Louvain",
     xlab = "Tamaño de comunidad (nodos)", ylab = "Frecuencia")

hist(sizes(cluster_girvan_newman),
     breaks = seq(0, max(sizes(cluster_girvan_newman))+2, by=2),
     col = "lightgreen", border = "white",
     main = "Distribución de tamaños - Girvan–Newman",
     xlab = "Tamaño de comunidad (nodos)", ylab = "Frecuencia")

hist(sizes(cluster_infomap),
     breaks = seq(0, max(sizes(cluster_infomap))+2, by=2),
     col = "salmon", border = "white",
     main = "Distribución de tamaños - Infomap",
     xlab = "Tamaño de comunidad (nodos)", ylab = "Frecuencia")
```

## Parte 3: Enriquecimiento Funcional (*Functional Enrichment*).
...
setwd(file.path("C:","Users", "Usuario_UMA", "Desktop")) # Change folder so we write everything to Desktop
